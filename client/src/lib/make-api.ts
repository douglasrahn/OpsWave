...
export async function startScenario(
  ...
  if (status.scenario.isActive) {
    console.log(
      "[Make.com API] Scenario is already active, skipping start request",
    );
    return status;
  }
...
export async function toggleScenario(
  ...
  console.log(
    `[Make.com API] Toggling scenario ${scenarioId} to ${activate ? "active" : "inactive"}`,
  );
  try {
    ...
    console.log(
      `[Make.com API] Successfully ${activate ? "started" : "stopped"} scenario:`,
      response,
    );
    return response;
  } catch (error) {
    console.error(
      `[Make.com API] Failed to ${activate ? "start" : "stop"} scenario:`,
      error,
    );
    throw error;
  }
